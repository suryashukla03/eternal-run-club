...
import { X, Upload, Route, Clock, Calendar, AlertCircle, Loader2, Check, Info, Share2 } from 'lucide-react'
...
export default function LogRunModal({ isOpen, onClose, userId, username = 'Runner', teamName = 'Alpha' }: LogRunModalProps) {
  ...
  const [successData, setSuccessData] = useState<{ points: number; imageUrl: string | null } | null>(null)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Success!
      setSuccess(true)
      setSuccessData({ points: validation.points, imageUrl: imageUrl })
      triggerConfetti()
  ...
  const handleShareToWhatsApp = () => {
    ...
    const paceStr = formatPace(calculatePace(parseFloat(distanceKm), totalDurationMins))
    
    const message = `🏃 *Eternal Run Club*

${username} just logged a run for Team ${teamName}!

📅 ${new Date(runDate).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}
📏 Distance: ${parseFloat(distanceKm).toFixed(2)} km
⏱️ Duration: ${durationStr}
⚡ Pace: ${paceStr} /km
🏆 Points earned: +${successData?.points || 0}

Check the leaderboard: ${window.location.origin}/leaderboard`
        ...
        {success ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-dark-400 mb-6">
              You earned <span className="text-green-400 font-bold">{successData?.points || estimatedPoints} points</span> for your team!
            </p>
            
            {/* Share to WhatsApp Button */}
            <button
              onClick={handleShareToWhatsApp}
              className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl bg-green-600 hover:bg-green-500 text-white font-medium transition-all mb-3"
            >
            ...
            <button
              onClick={handleClose}
              className="w-full py-3 px-4 rounded-xl bg-dark-700 hover:bg-dark-600 text-white font-medium transition-all"
            >
              Done
            </button>
          </div>
